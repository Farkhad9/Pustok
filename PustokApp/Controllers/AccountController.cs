...
namespace PustokApp.Controllers
{
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Login(LoginVm model)
        {
            ...
            {
                ModelState.AddModelError("", "Your account is locked. Please try again after 15 minutes.");
                return View(model);
            }
            ...
            if (!isPasswordValid)
            {
                ...
                {
                    ModelState.AddModelError("", $"Username or password is incorrect. {remainingAttempts} attempt(s) remaining before your account is locked.");
                }
                else
                {
                    ModelState.AddModelError("", "Your account has been locked due to multiple failed login attempts. Please try again after 15 minutes.");
            ...
            // Transfer basket items from cookie to database on login
        ...
        public async Task<IActionResult> UserProfile(string tab = "dashboard")
        {
            ...
            {
                UserProfileInfo = new UserProfileInfoVm
                {
        ...
        public async Task<IActionResult> UserProfile(UserProfileVm model)
        {
            ...
            // Update only FullName (Username should not be changed)
            user.FullName = model.UserProfileInfo.FullName;

            // Only update email if it's not empty and different from current
            if (!string.IsNullOrEmpty(model.UserProfileInfo.Email) && 
